...
network.add_conns(e1, e2, e3, e4)

generator.set_attr(eta=0.98)
# %%[sec_9]
compressor.set_attr(eta_s=0.85, pr=15)
...
for T in data['T_3']:
    ...
    eta['T_3'] += [abs(e4.E.val) / combustionChamber.ti.val * 100]

# reset to base value
c3.set_attr(T=1200)

for pr in data['pr']:
    compressor.set_attr(pr=pr)
    network.solve('design')
    power['pr'] += [abs(e4.E.val)]
    eta['pr'] += [abs(e4.E.val) / combustionChamber.ti.val * 100]

# reset to base value
compressor.set_attr(pr=15)
...
c5.set_attr(fluid={"CO2": 0.03, "O2": 0, "H2O": 0,
            "Ar": 0, "N2": 0, "CH4": None, "H2": None})
...
for ti in data:
    combustionChamber.set_attr(ti=ti)
    network.solve('design')
